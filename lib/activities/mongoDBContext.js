...
MongoDBContext.prototype.run = function (callContext, args) {
    ...
    function toConnectionsArray(conns) {
        ...
        function toConnection(conn) {
            ...
            if (_.isString(conn.url) && conn.url) return conn;
            throw new Error('Connection is invalid: ' + JSON.stringify(conn));
        }
    ...
    try {
        var conns = toConnectionsArray(connections);
        var processedConns = new StrMap();
        fast.forEach(conns, function (conn) {
        ...
        processedConns.forEachValue(function (conn) {
            connectPromises.push(Promise.promisify(MongoClient.connect)(conn.url, conn.options).then(function (db) {
                conn.db = db;
            }));
        });
    ...
    Promise.coroutine(function* () {
        ...
        try {
            ...
            fast.forEach(_.values(connections), function (db) {
                closeTasks.push(Promise.promisify(db.close, db)(true));
            });
            ...
            if (cursorCloseFails) console.warn("Cannot close all opened cursors. Number of failed attempts: " + cursorCloseFails + ".");
...
MongoDBContext.addCollectionToRecycleBin = function (activityScope, collection) {
    activityScope.get("MongoDBContext_CollectionRecycleBin")[collection.collectionName] = collection;
}

MongoDBContext.registerOpenedCursor = function (activityScope, cursor) {
    activityScope.get("MongoDBContext_OpenedCursors").push(cursor);
}

MongoDBContext.unregisterOpenedCursor = function (activityScope, cursor) {
    activityScope.set("MongoDBContext_OpenedCursors", _.without(activityScope.get("MongoDBContext_OpenedCursors"), cursor));
}
