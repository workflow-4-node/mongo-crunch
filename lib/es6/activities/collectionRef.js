...
CollectionRef.prototype._generateName = function(name) {
    let ttl = this.ttl;
    if (ttl && ttl > 0) {
        let to = new Date();
        to.addMinutes(ttl);
        name += "@" + to.toFormat("YYYY-MM-DD-HH-MI-SS");
    }
    return name;
};
...
CollectionRef.prototype.doWork = function (callContext) {
    let name = this.name;
    let mustExists = this.mustExists;
    let deleteOnExit = this.deleteOnExit;
    let clearBeforeUse = this.clearBeforeUse;
...
CollectionRef.prototype._varsGot = function (callContext, reason, result) {
    ...
    let self = this;
    let name = callContext.activity._generateName.call(this, result[0]);
